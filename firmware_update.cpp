...
#define DOWNLOAD_BUFFER_SIZE 1024  // Reduced from 8192 to prevent stack overflow


static String extractVersionFromTag(const String &tag);
static bool downloadFile(const String &url, const String &filepath);
static String buildDirectBinaryUrl(const String &baseUrl, const String &version);
...
static bool downloadFile(const String &url, const String &filepath)
{
    ...
    if (freeHeap < 50000) {  // Require at least 50KB free heap
    ...
    if (httpCode == HTTP_CODE_OK)
    {
        ...
        if (!error)
        {
            ...
            for (JsonObject asset : assets)
            {
                ...
                if (name.endsWith(".zip") && name.startsWith("msp-firmware"))
                {
                    if (name.indexOf("macos") >= 0)
                    {
                        macosUrl = asset["browser_download_url"].as<String>();
                        selectedAsset = name;
                    }
                    else if (selectedAsset.isEmpty())
                    {
                        selectedAsset = name;
                    }
                }
...
void vHalFirmware_testOTAManagement()
{
    ...
    String testFirmwareUrl = "https://github.com/A-A-Milano-Smart-Park/msp-firmware/releases/download/v3.4.0/msp-firmware-v3.4.0-macos.zip";
    String testFirmwarePath = "/firmware_test.zip";
    ...
    if (httpCode == HTTP_CODE_OK) {
        ...
        if (contentLength > 0) {
            ...
            if (firmwareFile) {
                ...
                File zipFile = SD.open(testFirmwarePath.c_str(), FILE_READ);
                if (zipFile) {
                    size_t zipSize = zipFile.size();
                    log_i("ZIP file size: %zu bytes", zipSize);
                    
                    // Read first few bytes to verify ZIP format
                    uint8_t header[4];
                    if (zipFile.readBytes((char*)header, 4) == 4) {
                        if (header[0] == 0x50 && header[1] == 0x4B && 
                            ...
                            log_e("FAIL: Invalid ZIP file format - Header: 0x%02X%02X%02X%02X", 
...
bool bHalFirmware_forceOTAUpdate(systemData_t *sysData, systemStatus_t *sysStatus, deviceNetworkInfo_t *devInfo)
{
    ...
    // GitHub API URL for latest release - try upstream repository first
    ...
    if (httpCode != HTTP_CODE_OK)
    {
        ...
        // Fallback: Create a test firmware file for testing OTA mechanism
        log_i("FALLBACK: Creating test firmware for OTA mechanism testing...");
        
        String testFirmwarePath = "/test_firmware.bin";
        File testFile = SD.open(testFirmwarePath.c_str(), FILE_WRITE);
        if (testFile) {
            // Create a minimal test firmware (just dummy data)
            uint8_t testData[1024];
            for (int i = 0; i < sizeof(testData); i++) {
                testData[i] = (uint8_t)(i % 256);
            }
            ...
            log_i("Created test firmware file: %s", testFirmwarePath.c_str());
            log_w("WARNING: This is just test data, not real firmware!");
            log_w("Testing OTA mechanism with dummy data...");
            
            // Test the OTA mechanism with dummy data (direct call for fallback testing)
            bool success = bHalFirmware_performOTAUpdate(testFirmwarePath);
            
            // Clean up test file
            SD.remove(testFirmwarePath.c_str());
            
            if (success) {
                log_i("OTA mechanism test completed (with dummy data)");
            } else {
                log_e("OTA mechanism test failed");
            }
            
            return success;
        } else {
            log_e("Failed to create test firmware file");
            return false;
        }
    }
