  ...
  if (SD.exists(configpath)) 
  {
    ...
    if (cfgfile) {
      ...
      String conftemplate =
        ...
        "#device_id=;\n"
        "#wifi_power=17dBm;\n"
        "#o3_zero_value=" + String(p_tData->ozoneData.o3ZeroOffset) + ";\n"
        "#average_measurements=" + String(pDev->avg_measurements) + ";\n"
        "#average_delay(seconds)=" + String(pDev->avg_delay) + ";\n"
        "#sea_level_altitude=" + String(p_tData->gasData.seaLevelAltitude) + ";\n"
        "#upload_server=;\n"
        "#mics_calibration_values=RED:" + String(p_tData->pollutionData.data.carbonMonoxide) + ",OX:" + String(p_tData->pollutionData.data.nitrogenDioxide) + ",NH3:" + String(p_tData->pollutionData.data.ammonia) + ";\n"
        "#mics_measurements_offsets=RED:" + String(p_tData->pollutionData.sensingResInAirOffset.redSensor) + ",OX:" + String(p_tData->pollutionData.sensingResInAirOffset.oxSensor) + ",NH3:" + String(p_tData->pollutionData.sensingResInAirOffset.nh3Sensor) + ";\n"
        "#compensation_factors=compH:" + String(p_tData->compParams.currentHumidity, 1) + ",compT:" + String(p_tData->compParams.currentTemperature, 3) + ",compP:" + String(p_tData->compParams.currentPressure, 4) + ";\n"
        "#use_modem=" + String(p_tSys->use_modem ? "true" : "false") + ";\n"
        "#modem_apn=;\n"
...
{ // builds a new logfile line and calls addToLog() (using logpath global var) to add it
  ...
  if (p_tSys->datetime) strftime(timeFormat, sizeof(timeFormat), "%Y-%m-%dT%T.000Z", &data->sendTimeInfo); // formatting date&time in TZ format
